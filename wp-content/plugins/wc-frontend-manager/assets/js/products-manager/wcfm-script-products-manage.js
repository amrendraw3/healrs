...
jQuery( document ).ready( function( $ ) {
  ...
  $('.page_collapsible').collapsible({
		...
		animateOpen: function(elem, opts) {
			...
			$('.collapse-close').find('span').removeClass('fa-arrow-circle-o-right').hide();
			elem.find('span').addClass('fa-arrow-circle-o-right').css( { 'float': 'right', 'padding': '5px' } ).show();
			$('.wcfm-container:not(:first)').stop(true, true).slideUp(opts.speed);
			elem.next().stop(true, true).slideDown(opts.speed);
		}
	});
	$('.page_collapsible').find('span').addClass('fa');
	$('.collapse-open').find('span').addClass('fa-arrow-circle-o-right').css( { 'float': 'right', 'padding': '5px' } ).show();
	...
	if( typeof gmw_forms != 'undefined' ) {
		...
		var shdescTinyMCE = tinymce.init({
																	...
																	plugins: [
																		...
																		'insertdatetime image media table contextmenu paste code geomywp'
																	],
		...
		if( $('#excerpt').hasClass('rich_editor') ) {
			var shdescTinyMCE = tinymce.init({
																		...
																		plugins: [
																			'advlist autolink lists link charmap print preview anchor',
																			'searchreplace visualblocks code fullscreen',
																			'insertdatetime image media table contextmenu paste code'
																		],
