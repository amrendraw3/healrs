  ...
  public $views_path;
	...
	public function load_scripts( $end_point ) {
      ...
      case 'wcfm-products-manage':
        ...
        // WC Booking Support
        if( wcfm_is_booking() ) {
		  		wp_enqueue_script( 'wcfm_wcbookings_products_manage_js', $this->js_lib_url . 'products-manager/wcfm-script-wcbookings-products-manage.js', array('jquery'), $WCFM->version, true );
		  	}
	  ...
	  switch( $end_point ) {
	    ...
	    case 'wcfm-products':
		  ...
		  case 'wcfm-products-manage':
		    ...
		    // WC Bookings Support
		    if( wcfm_is_booking() ) {
		  		wp_enqueue_style( 'wcfm_wcbookings_products_manage_css',  $this->css_lib_url . 'products-manager/wcfm-style-wcbookings-products-manage.css', array(), $WCFM->version );
		  	}
	...
	*/
	...
	function generateTaxonomyHTML( $taxonomy, $product_categories, $categories, $nbsp = '', $is_checklist = false, $is_custom = false, $is_hierarchical = true ) {
		global $WCFM;
		
		foreach ( $product_categories as $cat ) {
			$wcfm_allowed_categories = apply_filters( 'wcfm_allowed_categories', true, $taxonomy, $cat->term_id );
			$checklis_label_class = '';
			if( !$wcfm_allowed_categories ) $checklis_label_class = 'product_cats_checklist_item_hide_by_cap';
			if( $is_checklist ) {
				...
				if( $is_custom ) {
					echo '<label class="selectit">' . $nbsp . '<input type="checkbox" class="wcfm-checkbox" name="product_custom_taxonomies[' . $taxonomy . '][]" value="' . esc_attr( $cat->term_id ) . '"' . checked( in_array( $cat->term_id, $categories ), true, false ) . '/>' . esc_html( $cat->name ) . '</label>';
				} else {
					echo '<label class="selectit">' . $nbsp . '<input type="checkbox" class="wcfm-checkbox ' . $checklis_label_class . '" name="product_cats[]" value="' . esc_attr( $cat->term_id ) . '"' . checked( in_array( $cat->term_id, $categories ), true, false ) . '/><span>' . esc_html( $cat->name ) . '</span></label>';
				}
			} else {
				if( $wcfm_allowed_categories ) {
			...
			if( $is_hierarchical ) {
				...
				if ( $product_child_categories ) {
					...
					$this->generateTaxonomyHTML( $taxonomy, $product_child_categories, $categories, $nbsp . '&nbsp;&nbsp;', $is_checklist, $is_custom, $is_hierarchical );
